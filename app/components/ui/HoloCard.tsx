...
export default function HoloCard({ src, alt, className = '' }: HoloCardProps) {
    ...
    const applyTilt = useCallback((clientX: number, clientY: number) => {
        ...
        const rx = (0.5 - yRatio) * 30;
        const ry = (xRatio - 0.5) * 30;
            ...
            >
                <div className="holo-card-content">
                    ...
                    <img
                        ...
                        draggable={false}
                        className={`pointer-events-none transition-opacity duration-500 ease-out ${isImageLoaded ? 'opacity-100' : 'opacity-0'}`}
                    />
                </div>
            </div>
        </div>
